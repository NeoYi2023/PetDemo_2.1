// SPEC §9.8.16：竞技场「挑战」全屏弹层（JingJi-3），任意位置点击关闭。
...
namespace PetDemo.UI
{
    ...
    public sealed class ArenaChallengeOverlayView : MonoBehaviour
    {
        public const string ResChallengeSprite = "AirUI/JingJi-3";
        public const string OverlayObjectName = "ArenaChallengeOverlay";

        private static ArenaChallengeOverlayView instance;

        private RectTransform overlayRt;
        private RectTransform canvasRectCache;

        public static ArenaChallengeOverlayView GetOrCreate(RectTransform canvasRect)
        {
            if (canvasRect == null)
                return null;
            ...
            if (existing != null)
            {
                var existView = existing.GetComponent<ArenaChallengeOverlayView>();
                if (existView == null)
                    existView = existing.gameObject.AddComponent<ArenaChallengeOverlayView>();
            ...
            var sprite = Resources.Load<Sprite>(ResChallengeSprite);
            if (sprite != null)
            {
                bgImg.sprite = sprite;
            ...
            else
            {
                ...
                UnityEngine.Debug.LogWarning(
                    "[ArenaChallengeOverlayView] 缺少资源 Resources/" + ResChallengeSprite);
            }
            ...
            var view = go.AddComponent<ArenaChallengeOverlayView>();
            view.overlayRt = rt;
